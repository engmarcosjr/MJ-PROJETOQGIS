...
def run_export_pipeline(
    ...
    with tempfile.TemporaryDirectory(prefix="qgis_dxf_pro_") as temp_dir:
        ...
        try:
            for idx, layer in enumerate(layers):
                ...
                try:
                    ...
                    if temp_layer and temp_layer.featureCount() > 0:
                        ...
                        # Clonar simbologia e rotulagem para preservar estilos no DXF
                        if layer.renderer():
                        ...
                        # VITAL: Adicionar ao QgsProject (sem adicionar à árvore visual).
                        # Isso garante o registro do ciclo de vida C++ e do QObject cast interno do QgsDxfExport,
                        # evitando o crash de Segmentation Fault no QGIS.
                        project.addMapLayer(temp_layer, False)
                        temp_registered_layers.append(temp_layer.id())
            ...
            # Limpar as camadas temporárias registradas no QgsProject para liberar memória
            for lid in temp_registered_layers:
